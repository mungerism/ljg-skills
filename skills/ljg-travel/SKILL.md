...
**强制 NATIVE 模式。** 本 workflow 是多 skill 管道（Research → ContentAnalysis → ljg-card），不走 Algorithm 七步流程。运行环境不支持特定外部 research skill 时，退化为常规联网检索与内容提炼。
...
优先使用可用的深度研究能力完成一次高覆盖研究；若无专门的 research skill，则用常规联网检索完成同等信息收集。

**核心设计：不分"知识底图"和"平台发现"两步——它们是同一个研究操作的不同搜索角度。** 如环境支持并发，可并行覆盖学术/百科研究与平台内容搜索；否则顺序执行这两类搜索。
...
如环境支持并发，可对每个 URL 并行提炼；否则逐个处理。

对每个 URL 进行内容提炼，提取核心知识点。
