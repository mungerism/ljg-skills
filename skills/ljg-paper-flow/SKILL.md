...
一条命令完成：读论文 → 生成解读 → 铸成卡片。默认串行处理；只有在运行环境明确支持并发委托且不会增加失败面时，才并行处理多篇论文。
...
对每篇论文，按顺序执行两步。默认一个接一个处理，确保输出稳定可复现。

**步骤 A — 读论文（ljg-paper）：**

调用 `ljg-paper`，传入该论文的来源。等待完成，获得生成的 org 文件路径。

**步骤 B — 铸卡片（ljg-card）：**

读取步骤 A 生成的 org 文件，调用 `ljg-card`（默认 `-l`，或按用户指定的模具参数），以 org 文件内容为输入。等待完成，获得 PNG 文件路径。
